...
public class JCommander {
	...
	/**
	 * Description of a main parameter, which can be either a list of string or a
	 * single field. Both are subject to converters before being returned to the
	 * user.
	 */
	static class MainParameter {
		...
		public void addValue(Object convertedValue) {
			if (multipleValue != null) {
				...
				throw new ParameterException("Only one main parameter allowed but found several: " + "\"" + singleValue
						+ "\" and \"" + convertedValue + "\"");
			} else {
	...
	/**
	 * Messages for some of the exception messages
	 */
	 ...
	 * required=true and during the parsing phase, all the fields that are assigned
	 * a value are removed from it. At the end of the parsing phase, if it's not
	 * empty, then some required fields did not receive a value and an exception is
	 * thrown.
	 */
	 ...
	 *            The arguments to parse (optional).
	 *
	 * @deprecated Construct a JCommander instance first and then call parse() on
	 *             it.
	 */
	@Deprecated()
	 ...
	 *
	 * JCommander supports the {@code @file} syntax, which allows you to put all
	 * your options into a file and pass this file as parameter @param expandAtSign
	 * whether to expand {@code @file}.
	 */
	...
	/**
	 * @return a wrapper for a {@link java.io.PrintStream}, typically
	 *         {@link System#out}.
	 */
	...
	/**
	 * Sets the {@link ResourceBundle} to use for looking up descriptions. Set this
	 * to <code>null</code> to use description text directly.
	 */
	// declared final since this is invoked from constructors
	...
	private void validateOptions() {
		...
		if (!requiredFields.isEmpty()) {
			...
			throw new ParameterException(this.messages.getString("the.following") + pluralize(requiredFields.size(),
					this.messages.getString("option.is.required"), this.messages.getString("option.is.required.plural"))
					+ message);
		}
		...
		if (mainParameter != null && mainParameter.description != null) {
			...
			if (mainParameterDescription.getParameter().required() && !mainParameterDescription.isAssigned()) {
				throw new ParameterException(
						this.messages.getString("main.paramters.are.required")+"(\"" + mainParameterDescription.getDescription() + "\")");
			}

			// If the main parameter has an arity, make sure the correct number of
			...
			if (arity != Parameter.DEFAULT_ARITY) {
				...
				if (List.class.isAssignableFrom(value.getClass())) {
					...
					if (size != arity) {
						throw new ParameterException(
								this.messages.getString("there.should.be.exactly") + arity + this.messages.getString("main.paramters.but") + size + this.messages.getString("were.found"));
					}
				}
	...
	private boolean matchArg(String arg, IKey key) {
		String kn = options.caseSensitiveOptions ? key.getName() : key.getName().toLowerCase();
	...
	/**
	 * If arg is an option, we can look it up directly, but if it's a value, we need
	 * to find the description for the option that precedes it.
	 */
		...
		for (Parameterized parameterized : parameterizeds) {
			...
			if (wp != null && wp.getParameter() != null) {
				...
				if (p.names().length == 0) {
					...
					mainParameter.description = new ParameterDescription(object, p, parameterized, options.bundle,
							this);
				} else {
					ParameterDescription pd = new ParameterDescription(object, p, parameterized, options.bundle, this);
					for (String name : p.names()) {
						...
						if (p.required())
							requiredFields.put(parameterized, pd);
					}
				}
				...
				for (String name : dp.names()) {
					...
					p("Adding description for " + name);
					ParameterDescription pd = new ParameterDescription(object, dp, parameterized, options.bundle, this);
					fields.put(parameterized, pd);
					descriptions.put(new StringKey(name), pd);
			...
			if (!isDashDash && !"--".equals(a) && isOption(a) && jc == null) {
				...
				if (pd != null) {
					...
					if (options.acceptUnknownOptions) {
						...
						throw new ParameterException(this.messages.getString("unknown.option") + arg);
					}
				}
			} else {
				//
				...
				if ("--".equals(arg) && !isDashDash) {
					...
					if (mainParameter.annotation.converter() != null
					...
					for (final Class<? extends IParameterValidator> validator : mainParameter.annotation
							.validateWith()) {
					...
					//
					if (jc == null && validate) {
						throw new MissingCommandException(this.messages.getString("expected.main.parameter.got") + arg,
								arg);
					} else if (jc != null) {
	...
	private int processPassword(String[] args, int index, ParameterDescription pd, boolean validate) {
		...
		if (passwordArity == 0) {
			// password option with password not specified, use the Console to retrieve the
			// password
			...
			throw new ParameterException(this.messages.getString("password.parameter.at.most.one.argument"));
		}
	}
	...
	/**
	 * Init the main parameter with the given arg. Note that the main parameter can
	 * be either a List<String> or a single value.
	 */
	private void initMainParameterValue(String arg) {
		if (mainParameter == null) {
			throw new ParameterException(this.messages.getString("was.passed.main.parameter") + arg
					+ this.messages.getString("but.no.main.paramter.was.defined.in.your.arg.class"));
		}

		Object object = mainParameter.parameterized.get(mainParameter.object);
		Class<?> type = mainParameter.parameterized.getType();

		// If it's a List<String>, we might need to create that list and then add the
		// value to it.
	...
	public String getMainParameterDescription() {
		if (descriptions == null)
			createDescriptions();
		return mainParameter.annotation != null ? mainParameter.annotation.description() : null;
	}
	...
	/**
	 * Prints the usage on {@link #getConsole()} using the underlying
	 * {@link #usageFormatter}.
	 */
	public void usage() {
	...
	public static class Builder {
		...
		/**
		 * Sets the locale to use for the exception messages
		 * 
		 * @param locale
		 *            New Locale used throughout the programm
		 * @return
		 */
		public Builder locale(Locale locale) {
			this.jCommander.setLocale(locale);
			return this;
		}
		...
		/**
		 * Adds a factory to lookup string converters. The added factory is used prior
		 * to previously added factories.
		 * 
		 * @param factory
	 ...
	 *         class. This can be used to display the usage() in a different format
	 *         (e.g. HTML).
	 */
	 ...
	 * to previously added factories.
	 * 
	 * @param converterInstanceFactory
	...
	public Object convertValue(final Parameterized parameterized, Class type, String optionName, String value) {
		...
		if (type.isAssignableFrom(List.class) && converter == null) {
			// No list converter: use the single value converter and pass each parsed value
			// to it individually
			...
			converter = new DefaultListConverter(splitter, new IStringConverter() {
				...
				public Object convert(String value) {
					...
					return convertValue(parameterized,
							genericType instanceof Class ? (Class) genericType : String.class, null, value);
				}
			});
		}
			...
			if (!alias.equals(name)) {
				...
				if (mappedName != null && !mappedName.equals(progName)) {
					throw new ParameterException("Cannot set alias " + alias + " for " + name
							+ " command because it has already been defined for " + mappedName.name + " command");
				}
	...
	/**
	 * The name of the command or the alias in the form it was passed to the command
	 * line. <code>null</code> if no command or alias was specified.
	 *
	...
	/**
	 * @return the objects that JCommander will fill with the result of parsing the
	 *         command line.
	 */
	...
	private ParameterDescription findParameterDescription(String arg) {
		return FuzzyMap.findInMap(descriptions, new StringKey(arg), options.caseSensitiveOptions,
				options.allowAbbreviatedOptions);
	}

	private JCommander findCommand(ProgramName name) {
		return FuzzyMap.findInMap(commands, name, options.caseSensitiveOptions, options.allowAbbreviatedOptions);
	}

	private ProgramName findProgramName(String name) {
		return FuzzyMap.findInMap(aliasMap, new StringKey(name), options.caseSensitiveOptions,
				options.allowAbbreviatedOptions);
	}
	...
	public JCommander findCommandByAlias(String commandOrAlias) {
		...
		if (jc == null) {
			throw new IllegalStateException("There appears to be inconsistency in the internal command database. "
